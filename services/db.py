...
def add_new_book(title: str,
    ...
    if author_name:
        ...
        if author is not None:
            log.debug(f"Author: {author_name} exists, skipped")
        else:
            author = Author(name=author_name, picture='data/author/default.png', description='', is_stared=0)
            session.add(author)
        book.authors.append(author)
        log.debug(f"Add author: {author_name}")
